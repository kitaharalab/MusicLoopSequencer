...
class View(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.model = Model(self.line_height, self.block_width, self.line_block_height)
        self.createControl()
        self.drawLine()
        # 再生の状態
    ...
    def createView(self, master, width, height):
        ...
        control_frame = tk.LabelFrame(
            operation_frame, text="control", bg=operation_bg, fg=operation_fg
        )
        control_frame.place(relx=0.5, rely=0.2, anchor="center")
        # --
        option_frame = tk.LabelFrame(
            operation_frame, text="option", bg=operation_bg, fg=operation_fg
        )
        option_frame.place(relx=0.5, rely=0.45, anchor="center")
        # --
        analysis_frame = tk.LabelFrame(
            operation_frame, text="analysis", bg=operation_bg, fg=operation_fg
        )
        ...
        self.movie_frame = tk.Frame(
            master, width=movie_width, height=movie_height, bg=movie_bg
        )
        ...
        self.draw_frame = tk.Frame(
            master, width=self.draw_width, height=self.draw_height, bg=draw_bg
        )
    ...
    def createOpWidget(self, control_frame, option_frame, analysis_frame):
        ...
        for (
            fix_part,
            val,
        ) in fix_determine_list:
            tk.Radiobutton(
                option_frame,
                text=fix_part,
                indicatoron=0,
                width=10,
                command=self._fixClicked,
                variable=self.fix_determine_value,
                value=val,
            ).grid(row=0, column=count, padx=padx, pady=pady)
            count += 1
        ...
        for (
            constitution_part,
            val,
        ) in constitution_determine_list:
            tk.Radiobutton(
                option_frame,
                text=constitution_part,
                indicatoron=0,
                width=10,
                command=self._constitutionClicked,
                variable=self.constitution_determine_value,
                value=val,
            ).grid(row=1, column=count, padx=padx, pady=pady)
            count += 1
        ...
        display_movie_file_entry.grid(
            row=2, column=0, columnspan=3, padx=padx, pady=pady
        )
        # 進捗バー
        # ttkの色を変更
        s = ttk.Style()
        ...
        self.movie_analysis_progress_bar.grid(
            row=3, column=0, columnspan=3, padx=padx, pady=pady
        )
        # 読み込みボタン
    ...
    def createDrawZone(self, draw_frame):
        ...
        self.draw_canvas = tk.Canvas(
            ...
            height=self.draw_entire_height,
            scrollregion=(0, 0, self.draw_entire_width, self.draw_height),
        )
        # スクロールバー
    ...
    def createDrawBackground(self, draw_canvas):
        ...
        self.draw_canvas.create_rectangle(
            0, 0, self.draw_entire_width, self.line_height, fill=bg, width=0
        )
        self.draw_canvas.create_rectangle(
            0,
            self.line_height,
            self.draw_entire_width,
            self.draw_height,
            fill=bg,
            width=0,
        )
        ...
        for i in range(6):
            self.draw_canvas.create_line(
                0,
                self.line_block_height * i,
                self.draw_entire_width,
                i * self.line_block_height,
                fill=fg,
                width=thickness,
                tag="bg",
            )
        for i in range(4):
            self.draw_canvas.create_line(
                0,
                self.line_height + self.sound_block_height * i,
                self.draw_entire_width,
                self.line_height + self.sound_block_height * i,
                fill=fg,
                width=thickness,
    ...
    def createMovieZone(self, movie_frame):
        self.movie_canvas = tk.Canvas(
            self.movie_frame,
            width=self.movie_frame["width"],
            height=self.movie_frame["height"],
            bg="black",
        )
        ...
        for i, binary in enumerate(hmm_array, 0):
            ...
            for j, b in enumerate(binary, 0):
                if b == "1":
                    self.draw_canvas.create_rectangle(
                        i * self.block_width,
                        self.line_height + self.sound_block_height * j,
                        (i + 1) * self.block_width,
                        self.line_height + self.sound_block_height * (j + 1),
                        fill=bg_list[j],
                        width=0,
                        tag="sound",
                    )
    ...
    def drawSeekbar(self):
        ...
        while self.judge_play:
            ...
            # 取得したポジションを利用して描画を行う
            self.draw_canvas.create_line(
                self.draw_entire_width * p.get_position(),
                0,
                self.draw_entire_width * p.get_position(),
                self.draw_entire_height,
                fill="white",
                width=3,
                tag="position",
            )
    ...
    def displayVProgressOfAnalysis(self):
        ...
        while self.model.movie_analysising:
            self.movie_analysis_progress_bar[
                "value"
            ] = self.model.movie_analysis_progress
    ...
    def drawMovieLine(self, motion_excitement_array):
        ...
        for i in range(len(self.pixcel_array)):
            self.pixcel_array[i] = self.line_height - int(
                self.line_height * motion_excitement_array[count]
            )
    ...
    def drawSection(self, section_array):
        ...
        self.draw_canvas.create_line(
            intro_start * self.block_width,
            self.line_height,
            intro_end * self.block_width,
            self.line_height,
            fill="#d8bfd8",
            width=8,
            tag="section",
        ...
        self.draw_canvas.create_line(
            breakdown_start * self.block_width,
            self.line_height,
            breakdown_end * self.block_width,
            self.line_height,
            fill="#7b68ee",
            width=8,
            tag="section",
        ...
        self.draw_canvas.create_line(
            buildup_start * self.block_width,
            self.line_height,
            buildup_end * self.block_width,
            self.line_height,
            fill="#4682b4",
            width=8,
            tag="section",
        ...
        self.draw_canvas.create_line(
            drop_start * self.block_width,
            self.line_height,
            drop_end * self.block_width,
            self.line_height,
            fill="#191970",
            width=8,
            tag="section",
        ...
        self.draw_canvas.create_line(
            outro_start * self.block_width,
            self.line_height,
            outro_end * self.block_width,
            self.line_height,
            fill="#d8bfd8",
            width=8,
            tag="section",
    ...
    def _drawPressed(self, event):
        ...
        if (
            0 <= event.x
            and event.x < self.draw_width
            and 0 < event.y
            and event.y < self.line_height
        ):
    ...
    def _drawDragged(self, event):
        ...
        if (
            0 <= event.x
            and event.x < self.draw_entire_width
            and 0 < event.y
            and event.y < self.line_height
            and self.sx <= self.draw_entire_width
        ):
            self.pixcel_array[int(self.draw_canvas.canvasx(event.x))] = event.y
            ...
            if self.draw_canvas.canvasx(event.x) > self.sx:
                for i in range(1, difference):
                    self.pixcel_array[self.sx + i] = int(
                        self.sy
                        + (event.y - self.sy)
                        * (self.sx + i - self.sx)
                        / (self.draw_canvas.canvasx(event.x) - self.sx)
                    )
            else:
                for i in range(1, difference):
                    self.pixcel_array[self.sx - i] = int(
    ...
    def createMusic(self, array, projectid):
        ...
        if self.selected_fix_determine == 1:
            if self.selected_constitution_determine == 0:
                ...
                self.hmm_array, array = self.model.fixAutoHmm(
                    self.hmm_array, array, self.model.section_array
                )
        # 音素材を繋げる
    ...
    def _playClicked(self):
        ...
        if os.path.exists("./TechnoTrance/output.wav"):
            ...
            tkmsg.showerror(
                title="error", message="Push the create button or return key"
            )
    ...
    def _readClicked(self):
        ...
        if self.movie_file_path != "":
            ...
            read_executor.submit(
                self.model.analysisMovie, movie_file_path=self.movie_file_path
            )
