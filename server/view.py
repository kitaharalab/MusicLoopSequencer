...
class View(tk.Frame):
    ...
    def createView(self, master, width, height):
        ...
        operation_frame = tk.Frame(
            master, width=operation_width, height=operation_height, bg=operation_bg)
        ...
        description_frame = tk.Frame(
            master, width=description_width, height=description_height, bg=description_bg)
        description_frame.place(x=operation_width+self.draw_width)

        # フレームによって分割
        section_frame = tk.LabelFrame(
            description_frame, text="section", bg=description_bg, fg=description_fg)
        section_frame.place(relx=0.5, rely=0.3, anchor="center")
        # --
        part_frame = tk.LabelFrame(
            description_frame, text="part", bg=description_bg, fg=description_fg)
    ...
    def createDesWidget(self, section_frame, part_frame):
        ...
        # セクション
        intro_label = ttk.Label(section_frame, text='intro',
                                background='#d8bfd8', foreground='#ffffff', padding=(20, 10))
        intro_label.grid(row=0, column=0, padx=padx, pady=pady)
        breakdown_label = ttk.Label(section_frame, text='beakdown',
                                    background='#7b68ee', foreground='#ffffff', padding=(20, 10))
        breakdown_label.grid(row=1, column=0, padx=padx, pady=pady)
        buildup_label = ttk.Label(section_frame, text='buildup',
                                  background='#4682b4', foreground='#ffffff', padding=(20, 10))
        buildup_label.grid(row=2, column=0, padx=padx, pady=pady)
        drop_label = ttk.Label(section_frame, text='drop',
                               background='#191970', foreground='#ffffff', padding=(20, 10))
        drop_label.grid(row=3, column=0, padx=padx, pady=pady)
        outro_label = ttk.Label(section_frame, text='outro',
                                background='#d8bfd8', foreground='#ffffff', padding=(20, 10))
        outro_label.grid(row=4, column=0, padx=padx, pady=pady)

        # パート
        sequence_label = ttk.Label(
            part_frame, text='sequence', background='#40e0d0', foreground='#ffffff', padding=(20, 10))
        sequence_label.grid(row=0, column=0, padx=padx, pady=pady)
        synth_label = ttk.Label(
            part_frame, text='synth', background='#7fffd4', foreground='#ffffff', padding=(20, 10))
        synth_label.grid(row=1, column=0, padx=padx, pady=pady)
        bass_label = ttk.Label(
            part_frame, text='bass', background='#afeeee', foreground='#ffffff', padding=(20, 10))
        bass_label.grid(row=2, column=0, padx=padx, pady=pady)
        drums_label = ttk.Label(
            part_frame, text='drums', background='#5f9ea0', foreground='#ffffff', padding=(20, 10))
    ...
    def createDrawBackground(self, draw_canvas):
        ...
        self.draw_canvas.create_rectangle(
            0, self.line_height, self.draw_entire_width, self.draw_height, fill=bg, width=0)
    ...
    def drawMovieLine(self, motion_excitement_array):
        ...
        for i in range(len(self.pixcel_array)):
            self.pixcel_array[i] = self.line_height - \
                int(self.line_height*motion_excitement_array[count])
    ...
    def createMusic(self, array, projectid):
        ...
        if self.selected_fix_determine == 1:
            if self.selected_constitution_determine == 0:
                self.hmm_array, array = self.model.fixHmm(
                    self.hmm_array, array)
            else:
    ...
    def _playClicked(self):
        ...
        if os.path.exists("./TechnoTrance/output.wav"):
            ...
            play_executor = concurrent.futures.ThreadPoolExecutor(
                max_workers=3)
    ...
    def _readClicked(self):
        ...
        if self.movie_file_path != "":
            read_executor = concurrent.futures.ThreadPoolExecutor(
                max_workers=2)
            read_executor.submit(self.model.analysisMovie,
    ...
    def _reflectClicked(self):
        ...
        if len(self.model.motion_excitement_array) >= 4:
            ...
            tkmsg.showerror(
                title="error", message="not movie file or movie too short")
        self.createMusic()
